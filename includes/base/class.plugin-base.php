...
abstract class Plugin_Base {
	...
	public static function getOption( string $option_name, $default_value = '' ) {
		$option = get_option( WPSP_PLUGIN_PREFIX . '_' . $option_name );
	...
	public static function addOption( string $option_name, $option_value ): bool {
		return add_option( WPSP_PLUGIN_PREFIX . '_' . $option_name, $option_value );
	}
	...
	public static function updateOption( string $option_name, $option_value ): bool {
		return update_option( WPSP_PLUGIN_PREFIX . '_' . $option_name, $option_value );
	}
	...
	public static function deleteOption( string $option_name ): bool {
		return delete_option( WPSP_PLUGIN_PREFIX . '_' . $option_name );
	}
	...
	public function render_template( $template_name, $args = [] ) {
		$template_name = apply_filters( WPSP_PLUGIN_PREFIX . '/template/name', $template_name, $args );

		$path = WPSP_PLUGIN_DIR . "/templates/$template_name.php";
		if ( file_exists( $path ) ) {
			ob_start();
			include $path;

			echo apply_filters( WPSP_PLUGIN_PREFIX . '/content/template', ob_get_clean(), $template_name, $args ); // @codingStandardsIgnoreLine
		} else {
			echo apply_filters( WPSP_PLUGIN_PREFIX . '/message/template_not_found', __( 'This template does not exist!', 'pro-social-poster' ) ); // @codingStandardsIgnoreLine
		}
	}
}
