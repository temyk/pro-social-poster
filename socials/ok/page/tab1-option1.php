...
<?php if ( empty( $okposter_aid ) ) { ?>
    <p>
        <a class="button" href="<?php echo OKPOSTERBASE::URL_OK_DEVCREATE; ?>" target="_blank"
           title="После прохождения вы получите ID приложения, который нужно будет ввести в поле ниже и сохранить">Создать
            приложение</a>
    </p>
<?php } ?>
<p>&nbsp;</p>
<form method="post" action="options.php">
    ...
    <table class="form-table">
        <tr valign="top">
            ...
            <td>
                <input id="idsofttip" title="Application ID из письма, после создания приложения" type="text"
                       pattern="[0-9]*" name="okposter_aid" value="<?php echo $okposter_aid; ?>"/>
            </td>
        </tr>
        <tr valign="top">
            <th scope="row">Токен приложения (access_token)</th>
            <td>
                <input id="tekentip" title="Это поле вы заполните данными пункта Вечный access_token" size="90"
                       type="text" name="okposter_accesstoken" value="<?php echo $okposter_accesstoken; ?>"/>
            </td>
        </tr>
        <tr valign="top">
            <th scope="row">Секретный ключ приложения</th>
            <td>
                <input id="toltipidsec" title="Введите сюда Секретный ключ приложения из письма" type="text"
                       name="okposter_seckey" value="<?php echo $okposter_seckey; ?>"/>
            </td>
        </tr>
        <tr valign="top">
            <th scope="row">Публичный ключ приложения</th>
            <td>
                <input id="toltipidsec" title="Введите сюда Публичный ключ приложения из письма" type="text"
                       name="okposter_pubkey" value="<?php echo $okposter_pubkey; ?>"/>
            </td>
        </tr>
        <tr valign="top">
            <th scope="row">ID группы (gid)</th>
            <td>
                <input id="toltipid1" title="Введите сюда ID группы" type="text" name="okposter_gid"
                       value="<?php echo $okposter_gid; ?>"/>
            </td>
        </tr>
        ...
        <tr valign="top">
            ...
            <td>
                <select name="okposter_text_link">
                    ...
                    <option value="1" <?php selected( $okposter_text_link, '1', true ); ?>>Разворачивать ссылку
                        (картинка, заголовок, часть текста)
                    </option>
                </select>
            </td>
        </tr>
        ...
        <tr valign="top">
            ...
            <td>
                <input id="toltiptitle" title="Количество слов для отправки на стену" type="text" pattern="[-0-9]*"
                       name="okposter_counttext" value="<?php
				...
				?>"/>
                <p class="description">Количество отправляемых слов. Без ограничений — 0. Не отправлять текст —
                    -1.".</p>
            </td>
        </tr>
        ...
        <tr valign="top">
            ...
            <td>
                <fieldset>
					...
					foreach ( $array_posts as $v ) {
                        ...
                        <label for="cbox-<?php echo $v; ?>">
                            <input type="checkbox" name="okposter_posttype[<?php echo $v; ?>]"
                                   id="cbox-<?php echo $v; ?>" value="<?php echo $v; ?>" <?php
                ...
                <p class="description">Выберите типы «записей» при добавление которых будет работать функция отправки в
                    OK.ru. По умолчанию всегда активен тип записей «Post». Если вам необходимо что бы была возможность
                    отправлять данные из «Произвольных типов записей» поставте напротив «галочку». Если вы не знаете что
                    такое «Произвольный тип записей» - ни чего не трогайте.</p>
            </td>
        </tr>
    </table>
    ...
    <input type="hidden" name="page_options"
           value="okposter_aid, okposter_accesstoken, okposter_seckey, okposter_id, okposter_pubkey, okposter_text_link, okposter_gid, okposter_counttext, okposter_onoff, okposter_posttype"/>
    <p class="submit">
