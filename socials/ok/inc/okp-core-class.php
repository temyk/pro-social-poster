...
class OKPOSTERBASE {
	...
	const PATCH_PLUGIN = WPSP_PLUGIN_SOCIALS_DIR . '/ok';
	...
	public function addOptions() {
		...
		add_option( 'okposter_jornal', [] );
		add_option( 'okposter_posttype', [ 'post' => 'post' ] ); //Типы выбранных записей
	}
	...
	public function addActios() {
		...
		add_action( 'add_meta_boxes', [ $this, 'settingMetabox' ] ); //Добавляем метабокс в пост

		$array_posts = get_option( 'okposter_posttype' ); //Типы постов
		foreach ( $array_posts as $k => $v ) {
			add_action( 'save_' . $v, [ $this, 'metaboxSavePost' ], 9 ); //Сохранение галки в постах
			//add_action('publish_' . $v, array($this, 'metaboxSavePost'), 9); //Сохранение галки в постах
			add_action( 'publish_' . $v, [ $this, 'metaboxSentOK' ], 11 ); //Отправка значений На стену ВК
			//add_action('publish_future_' . $v, array($this, 'futureSentOk'), 10, 2); //Публикация отложенной записи

			//add_action('future_to_publish', array($this, 'futureSentOk'), 10); //Публикация отложенной записи
			add_action( 'transition_post_status', [ $this, 'transitionSentOk' ], 10, 3 );
		}

		//фильтры для добавления ссылки отправки записи
		add_filter( 'post_row_actions', [ $this, 'okp_action_row' ], 10, 2 );
		add_filter( 'page_row_actions', [ $this, 'okp_action_row' ], 10, 2 );
		...
		add_action( 'admin_head-edit.php', [ $this, 'admin_head_post_listing' ] );
	}
	 ...
	 * @return type
	 */
	public function futureSentOk( $post_id ) {
	 ...
	 * Отправка записи
	 */
	public function transitionSentOk( $new_status, $old_status, \WP_Post $post ) {
	...
	public function admin_head_post_listing() {
		...
		if ( in_array( $post->post_type, get_option( 'okposter_posttype' ) ) and isset( $_GET['okp_repost'] ) ) {
			...
			$okpunk->logJornal( $post_id . '_link', $post->post_title, $status_sent ); //Логируем результаты
	...
	public function adminOptions() {
		$page_option = add_options_page( self::NAME_TITLE_PLUGIN_PAGE, self::NAME_MENU_OPTIONS_PAGE, 'edit_posts', self::URL_ADMIN_MENU_PLUGIN, [
			$this,
			'showSettingPage',
		] );
	}
	...
	public function showSettingPage() {
		include_once self::PATCH_PLUGIN . '/' . self::OPTIONS_NAME_PAGE;
	}
	...
	public function settingMetabox() {
		...
		foreach ( $array_posts as $k => $v ) { //Появляется только там где выбрал пользователь
			add_meta_box( 'okposter-metabox', self::NAME_SERVIC_ORIGINAL_TEXT, [
				$this,
				'metaboxHtml',
			], "$v", 'side', 'high' );
		}
	...
	public function metaboxSentOK( $post_id ) {
		...
		if ( $chek == 'on' ) {
			...
			if ( $difference < 5 ) {
				$status_sent = $okpunk->setOkWall( $post_id ); //Отправка текста
				$okpunk->logJornal( $post_id . '_editPub', $title, $status_sent ); //Логируем результаты
			} elseif ( $difference > 5 and $radio_chek !== '1' ) {
				$status_sent = $okpunk->setOkWall( $post_id ); //Отправка текста
				$okpunk->logJornal( $post_id . '_editUpd', $title, $status_sent ); //Логируем результаты
			}
	...
	static public function tabViwer() {
		...
		switch ( $tab ) {
			case 'jornal':
				include_once self::PATCH_PLUGIN . '/page/tab2-option1.php';
				break;
			case 'help':
				include_once self::PATCH_PLUGIN . '/page/tab3-option1.php';
				break;
			default :
				include_once self::PATCH_PLUGIN . '/page/tab1-option1.php';
		}
	}
