...
class CorePlugin {
	...
	const PATCH_PLUGIN = WPSP_PLUGIN_SOCIALS_DIR . '/vk'; //Директория плагина
	...
	public function addAdminActions() {
		...
		add_filter( 'plugin_action_links', array(
			$this,
			'pluginLinkSetting'
		), 10, 2 ); //Настройка на странице плагинов
	...
	public function adminOptions() {
		$page_option = add_options_page( self::NAME_TITLE_PLUGIN_PAGE, self::NAME_MENU_OPTIONS_PAGE, 'edit_posts', self::URL_ADMIN_MENU_PLUGIN, array(
			$this,
			'showSettingPage'
		) );
		add_action( 'admin_print_styles-' . $page_option, array(
			$this,
			'adminStyles'
		) ); //загружаем стили только для страницы плагина
	...
	public function view_tab_page() {
		$tab = $this->get_tab_numger();
		if ( \file_exists( self::PATCH_PLUGIN . $this->plugin_admin_pages[ $tab ]['path'] ) ) {
			include_once self::PATCH_PLUGIN . $this->plugin_admin_pages[ $tab ]['path'];
		} else {
