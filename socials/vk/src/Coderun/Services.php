...
class Services {
	 ...
	 *
	 * @param string $method Полный метод ВК, с УРЛ и знаком ?
	...
	public function sentRequesVK( $method, $arg, $proxy = null ) {
		...
		if ( ! empty( $proxy ) ) { //Если прокси, то отправляем по старому
			$url  = $method . $query;
			$curl = curl_init();
			...
			$curlinfo = wp_remote_post( $method, [ 'body' => $query, 'timeout' => 10 ] );
...
//В перспективе изображение по умолчанию (сейчас не реализованно)
	...
	protected function getUrlPostVK( $vkPostId ) {
		...
		$mask = sprintf( 'https://vk.com/wall-%s_%s', $group, $vkPostId ); // стена пользователя
